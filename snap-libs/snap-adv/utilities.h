...
namespace Utils {
	...
	int kDFSFromNode(PUNGraph graph, const int depthlimit, const int startingNode, std::unordered_set<staticEdge_t, DataStructures::hash_pair>& subgraph);
	int kBFSFromNode(PUNGraph graph, const int depthlimit, const int startingNode, std::unordered_set<staticEdge_t, DataStructures::hash_pair>& subgraph);
	...
	int GetG6FromEdge(const PUNGraph graph, const std::pair<node_t,node_t>& sampledEdge, std::vector<std::vector<std::pair<node_t, node_t> > >& triangles);
	...
	int GetSquaresFromNode(PUNGraph graph, const int startingNode, std::vector<std::vector<std::pair<node_t, node_t> > >& squares);
	int GetEdgesFromNode(PUNGraph graph, const int startingNode, std::vector<std::vector<std::pair<node_t, node_t> > >& edges);
	int GetDirectedTrianglesFromNode(PNGraph graph, const int startingNode, std::vector<std::vector<std::pair<node_t, node_t> > >& triangles, bool isCycle);


	double GetSampledWedge(PUNGraph graph, const std::pair<node_t,node_t>& sampledEdge, std::vector<std::vector<std::pair<node_t, node_t> > >& squares, TVec< THash<TInt, TInt64 >>& mapEdgeIdx, const std::vector<size_t>& weights, std::mt19937& generator, int alpha);
}
